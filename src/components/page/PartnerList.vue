        ...
        <div class="panel-body">
            <div class="panel-heading searchDiv">
                <el-row class="page-wrap">
                    <el-input v-model="name" placeholder="合作方名称模糊查询"></el-input>
                    <div class="grid-content bg-purple btn-group">
                        <el-button type="primary" @click="search()">查询</el-button>
                        <router-link :to="{name: 'PartnerAdd', params: {pageStatus:'add', id: ''}}">
            ...
            <el-table
                :data="tableData"
                style="width: 100%">
                <el-table-column label="合作方名称" prop="name"></el-table-column>
                <el-table-column label="类型" prop="partner_type" :formatter="formatType"></el-table-column>
                <el-table-column label="父合作方名称" prop="fatherName"></el-table-column>
                <el-table-column label="创建时间" prop="ctime"></el-table-column>
                <el-table-column label="操作">
                    <template slot-scope="scope">
                        <router-link :to="{name: 'PartnerAdd', params: {pageStatus:'edit', id: scope.row.id  }}">
    ...
    export default {
        ...
        methods: {
            getTablePageData(pagerObj) {
                ...
                console.log(params)
                let that = this
                that.$axios.get('/api/partner/list', {params: params}).then(function (res) {
            ...
            del (val, index) {
                ...
                that.$confirm('此操作将永久删除, 是否继续?', '提示', {
                    ...
                    that.$axios.get('/api/partner/del/' + val).then(function (res) {
                        if (res.status === 200 && res.data.code === 200) {
        ...
        mounted () {
            let that = this
            that.$axios.get('/api/partner/list').then(function (res) {
                if (res.status === 200 && res.data.code === 200) {
                    that.tableData = res.data.data.tableData
                    that.totalNum = res.data.data.totalNum
                }
